...
class custom_dataset(Dataset):
    def __init__(self, dataset_folder_path, transform, data, first_n_days, balance_data=False, n_train_labels=None):
        ...
        for i in range(len(self.data)):
            ...
            for subset in subsets:
                if n_days == "all":
                    _images = glob(f"{self.dataset_folder_path}/{subset}/**/*.jpg", recursive=True)
                    _labels = [0 if "empty" in img_path else 1 for img_path in _images]
                    ...
                    for day in selected_days:
                        _images.extend(glob(f"{self.dataset_folder_path}/{subset}/{day}/**/*.jpg", recursive=True))
                    _labels = [0 if "empty" in img_path else 1 for img_path in _images]
                    images.extend(_images)
                    labels.extend(_labels)
            
            if self.n_train_labels is not None and self.n_train_labels[i] != 'all':
                n_labels = int(self.n_train_labels[i])
                combined = list(zip(images, labels))
                shuffle(combined)
                images, labels = zip(*combined)
                images = list(images[:n_labels])
