...
def train(model):
    ...
    while (1):
        for batch in model.get_train_dataloader():
            ...
            actual_step += 1
            if actual_step % (model.get_num_steps() // 10) == 0  or actual_step == model.get_num_steps():
...
def handle_args(args):
    ...
    if not os.path.exists(args.encoder_weights) and args.encoder_weights.lower() not in ['imagenet', 'none']:
        raise FileNotFoundError(f"Encoder weights {args.encoder_weights} does not exist.")
