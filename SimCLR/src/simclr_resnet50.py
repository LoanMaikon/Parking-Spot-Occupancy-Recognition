...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.encoder_out_features = 512 * block.expansion * width_mult
    ...
    def load_weights(self, weight_path, device):
        # First trying with classifier head with num_classes=2
        self.fit_classifier_head(num_classes=2)
        try:
            self.load_state_dict(torch.load(weight_path, map_location=device, weights_only=True))
        except:
            # If it fails, trying with classifier head with num_classes=1000
            self.fit_classifier_head(num_classes=1000)

            try:
                self.load_state_dict(torch.load(weight_path, map_location=device, weights_only=True))
            except:
                # If it fails, trying with projection head
                self.fit_projection_head()
...
def simclr_resnet50(use_checkpoint=True):
